...
func (e *ExportAsset) executeReturn(
	...
	allowedDestination, err := ids.ToID(metadata[hconsts.IDLen:])
	if err != nil {
		return false, ExportAssetComputeUnits, utils.ErrBytes(err), nil, nil
	}
	...
	originalAsset, err := ids.ToID(metadata[:hconsts.IDLen])
	if err != nil {
		return false, ExportAssetComputeUnits, utils.ErrBytes(err), nil, nil
	}
	wt := &WarpTransfer{
...
func (*ExportAsset) Size() int {
	return codec.AddressLen + hconsts.IDLen +
		hconsts.Uint64Len + hconsts.BoolLen +
		hconsts.Uint64Len + /* op bits */
		hconsts.Uint64Len + hconsts.Uint64Len + hconsts.IDLen + hconsts.Uint64Len +
		hconsts.Int64Len + hconsts.IDLen
}
...
func (e *ExportAsset) Marshal(p *codec.Packer) {
	...
	p.PackBool(e.Return)
	op := codec.NewOptionalWriter(hconsts.Uint64Len*3 + hconsts.Int64Len + hconsts.IDLen)
