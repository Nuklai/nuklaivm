...
import { getBalance, isFaucetReady, vmClient } from '../VMClient'
...
export default function ConnectWallet() {
  ...
  async function connectWallet(
    ...
    snapSource: 'npm' | 'local' = 'npm'
  ) {
    try {
      setLoading((prevLoading) => prevLoading + 1)
      if (signerType === 'metamask-snap') {
        const snapId =
          snapSource === 'local' ? 'local:http://localhost:8989' : undefined
    ...
    return (
      <div className='flex items-center justify-center min-h-screen'>
        <div className='text-2xl font-bold'>
          Please confirm the connection in Metamask Flask signer
        </div>
      </div>
    )
  }
  ...
  if (errors.length > 0) {
    return (
      <div className='border border-black  p-6 rounded-lg max-w-md w-full '>
        ...
        <button
          className='px-4 py-2 bg-black text-white font-bold rounded hover:bg-gray-800 transition-colors duration-200 transform hover:scale-105'
    ...
    <div className='flex items-center justify-center min-h-screen'>
      <div className='border border-black px-16 py-12 rounded-lg max-w-3xl w-full text-center'>
        ...
        <p className='mt-4 text-sm'>
          Connect with Metamask Flask development signer via a Snap, or create a
          signer in memory.
        </p>
        <div className='mt-8 flex flex-col space-y-4'>
          <div className='flex flex-row justify-between space-x-4'>
            <button
              ...
              } text-white font-semibold rounded transition-colors duration-200 transform ${
                isFlaskInstalled ? 'hover:scale-105' : ''
              }`}
              onClick={() =>
                isFlaskInstalled && connectWallet('metamask-snap', 'npm')
              }
              disabled={!isFlaskInstalled}
            >
              {isFlaskInstalled
                ? 'Connect with Snap'
                : 'Mo MetaMask Flask detected'}
            </button>
            <button
              type='button'
...
function IsReadyWidget() {
  const [faucetStatus, setFaucetStatus] = useState<
    'loading' | 'ready' | 'error'
  >('loading')
  const [vmApiStatus, setVmApiStatus] = useState<'loading' | 'ready' | 'error'>(
    'loading'
  )
  ...
  return (
    <div className='flex justify-between items-center px-4 py-2 bg-gray-100 rounded-lg'>
      <span className='text-sm'>
        {getStatusWithEmoji('Faucet', faucetStatus)}
      </span>
      <span className='text-sm'>
        {getStatusWithEmoji('VM API', vmApiStatus)}
      </span>
      <span className='text-sm'>
        {getStatusWithEmoji('Core API', coreApiStatus)}
      </span>
    </div>
  )
}
